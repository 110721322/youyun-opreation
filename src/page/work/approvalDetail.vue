<template>
    ...
    <detailMode :ruleForm="ruleForm" :configData="configData" :isShowCallBtn='true' :spanWidth="24">
            ...
            <el-step v-for="(item,index) in stepData" :key="index">
                <template slot="icon">
                    <div class="step-border">
                        ...
                        <i class="el-icon-check step-check" v-if="item.isCheck"></i>
                    </div>
                </template>
                <template slot="title">
                    <div class="step-title" v-if="item.title" :class="[item.isApproval?'step-title_blue':item.isUndo?'step-title_gray':'step-title_normal']">
                        {{item.title}}
                    </div>
                </template>
                <template slot="description">
                    <div class="step-description" v-if="item.name">
                        {{item.name}}
                    </div>
                    <div class="step-time" v-if="item.time">
                        {{item.time}}
                    </div>
                    <div class="step-note" v-if="item.note">
                        {{item.note}}
                    </div>
                </template>
            </el-step>
        ...
        <template v-slot:operatingTop>
            <span class="task-time status">
                {{status+":"}} <transformTime :time="new Date(new Date().getTime()-3600*24*3*1000)"></transformTime>
            </span>
        </template>
        <template v-slot:operatingBottom>
            <div class="opearting-bottom">
                <el-button type="primary" class="btn_oper">
                    同意
                </el-button>
                <el-button plain class="btn_oper btn_operno">
                    拒绝
                </el-button>
            </div>
        </template>
